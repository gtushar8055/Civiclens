...
function Login() {
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    try {
      const response = await BACKEND_API.post("/api/auth/login", {
        email,
        password,
      });
      ...
      >
        <div className="bg-white dark:bg-slate-800 rounded-[32px] p-8 md:p-10 shadow-floating border-[3px] border-slate-100 dark:border dark:border-slate-700 relative overflow-hidden">
          ...
          <div className="text-center mb-8">
            <div className="w-16 h-16 rounded-full bg-purple-100 dark:bg-purple-900/30 flex items-center justify-center mx-auto mb-6 text-purple-600 dark:text-purple-400">
              <UserCircle size={32} />
            </div>
            <h1 className="text-3xl font-bold font-heading mb-2 text-slate-900 dark:text-white">Welcome Back</h1>
            <p className="text-slate-500 dark:text-slate-400 font-medium">Log in to manage your reports</p>
          </div>
